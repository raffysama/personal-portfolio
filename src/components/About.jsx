function About({ year, text, company, description, gray }) {
  return (
    <div className="relative pl-10 group">
      {/* vertical line */}
      <div className="absolute left-3 top-7 h-[150px] w-px bg-gray-300 group-hover:bg-[#06a77d] transition-colors"></div>

      {/* dot */}
      <div
        className={`absolute left-[7px] top-1 w-3 h-3 rounded-full transition-transform group-hover:scale-125
        ${gray ? "bg-[#06a77d]" : "bg-gray-400"}`}
      ></div>
      ...
      <div className="flex items-center">
        <span
          ...
          ${gray ? "text-[#06a77d]" : "text-gray-400"}`}
        >
          {year}
        </span>

        <div className="flex-1 h-px bg-gray-300 mx-3 group-hover:bg-[#06a77d] transition-colors"></div>
      </div>
